...
/**
* Checks if a list of fields contains a certain field.
* @param {object} fields - Fields object to check
* @param {string} field - Field to check for.
* @returns {boolean} - New JSON object
* @function utils.json.containsField
*/
exports.containsField = function (fields, field) {
  var keys = [];

  //extract all the fields.
  for(var key in fields){
    if(fields.hasOwnProperty(key) && key !== '_id'){
      keys.push(key);
    }
  }

  //check if we have the field or are of length 0.
  return (keys.length === 0 || keys.indexOf(field) !== -1);
};
