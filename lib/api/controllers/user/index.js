...
var completions = require('../../../studentModel').completions;
...
exports.me = function(req, res){
  if(req.user){
    findOneUser(req, res, {username: req.user.username});
  } else {
    res.errorJson('UserNotFound');
  }
};
...
exports.image = function (req, res) {
  ...
  .exec(function (err, student) {
    ...
    //and pipe the image.
    request.get(settings.imageBaseUrl.replace(":eid", student.eid)).pipe(res);
  });
};
...
var autocomplete = (function(){
  ...
  for(var key in completions){
    if(completions.hasOwnProperty(key)){
      complete = completions[key];
      if(Array.isArray(complete)){
        _autocomplete[key] = complete;
      } else {
        _autocomplete[key] = '';
      }
    }
...
exports.autocomplete = function(req, res){
  res.status(200).jsonp(autocomplete);
};
