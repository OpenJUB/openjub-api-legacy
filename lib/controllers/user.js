...
var execQueries = function(req, fields, queries, limit, skip, cb){
...
exports.autocomplete = function (req, res, next) {
  ...
  if (query.q.toLowerCase() === 'favorites' && req.access && req.access.role === 'user') {
    ...
    //and then in general.
    var queryFull = jsonUtils.map_value(searchQuery, function(v){return new RegExp("(?:^|\\s)"+v+"(?:$|\\s)", "i")});
    var queryStart = jsonUtils.map_value(searchQuery, function(v){return new RegExp("(?:^|\\s)"+v, "i")});
    var queryEnd = jsonUtils.map_value(searchQuery, function(v){return new RegExp(v+"(?:$|\\s)", "i")});
    var query = jsonUtils.map_value(searchQuery, function(v){return new RegExp(v, "i")});
